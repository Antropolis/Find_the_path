...
def main():
    ...
    while True:
        ...
        if cur_pos.y + 1 < LEN and not checked[depth][0] and not visited_numbers[labyrinth[cur_pos.y + 1][cur_pos.x] - 1]:
            checked[depth][0] = True
            depth += 1
            cur_pos.y += 1

        elif cur_pos.x + 1 < LEN and not checked[depth][1] and not visited_numbers[labyrinth[cur_pos.y][cur_pos.x + 1] - 1]:
            checked[depth][1] = True
            depth += 1
            cur_pos.x += 1

        elif cur_pos.y - 1 >= 0 and not checked[depth][2] and not visited_numbers[labyrinth[cur_pos.y - 1][cur_pos.x] - 1]:
            checked[depth][2] = True
            depth += 1
            cur_pos.y -= 1

        elif cur_pos.x - 1 >= 0 and not checked[depth][3] and not visited_numbers[labyrinth[cur_pos.y][cur_pos.x - 1] - 1]:
